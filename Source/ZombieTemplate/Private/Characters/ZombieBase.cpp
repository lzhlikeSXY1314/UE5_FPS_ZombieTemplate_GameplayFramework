...
FActorSaveData AZombieBase::GetSaveData_Implementation() const
{
    ...
    // 保存断肢数据：从 DismembermentComponent 获取已切断的骨骼列表
    UDismembermentComponent* DismemberComp = FindComponentByClass<UDismembermentComponent>();
    if (DismemberComp)
    {
        // MissingLimbs 是一个 TMap<FName, USkeletalMeshComponent*>，键是骨骼名
...
void AZombieBase::RestoreState_Implementation(const FActorSaveData& Data)
{
    ...
    // 4. 根据存档数据隐藏断肢骨骼（只隐藏，不生成断肢组件）
    UDismembermentComponent* DismemberComp = FindComponentByClass<UDismembermentComponent>();
    if (DismemberComp)
    {
        // 先把 ResetToDefault 清空的 MissingLimbs 恢复为存档中的断肢记录
        for (const FName& Bone : Data.DismemberedBones)
        {
            // 映射到 nullptr，因为断肢组件已经不存在了，但需要这个键来阻止再次断肢
    ...
    if (Data.bIsDead)
    {
        bIsDeath = false;   // 确保 HandleDeathFunc 能进入
        HandleDeathFunc(FVector::ZeroVector);
    }
    // 存活状态已由 ResetToDefault 处理完毕，骨骼已被隐藏
}
...
void AZombieBase::ResetToDefault_Implementation()
{
    ...
    if (TargetDismemberMesh)
    {
        ...
        // 重建物理约束（修复 BreakConstraint 造成的断裂）
...
void AZombieBase::HandleDeathFunc(FVector InShotFromDirection)
{
    ...
    if (UCharacterMovementComponent* MoveComp = GetCharacterMovement())
    {
        ...
        MoveComp->DisableMovement();   // 或 SetActive(false)
    }
